    ...
    protected void doFilterInternal(@NonNull HttpServletRequest request, @NonNull HttpServletResponse response,
        ...
        try {
            ...
            WebUtils.renderString(HttpServletResponse.SC_FORBIDDEN, ContentType.JSON.getValue(), response,
                    JSONUtil.toJsonStr(CommonResult.error(ResultCodeEnum.AUTH_FAIL.getCode(), "token 已过期")));
        } catch (UnsupportedJwtException | MalformedJwtException e) {
            WebUtils.renderString(HttpServletResponse.SC_FORBIDDEN, ContentType.JSON.getValue(), response,
                    JSONUtil.toJsonStr(CommonResult.error(ResultCodeEnum.AUTH_FAIL.getCode(), "token 认证失败")));
        } catch (IllegalArgumentException e) {
            WebUtils.renderString(HttpServletResponse.SC_FORBIDDEN, ContentType.JSON.getValue(), response,
                    JSONUtil.toJsonStr(CommonResult.error(ResultCodeEnum.AUTH_FAIL.getCode(), "token 不能为空")));
        }
    }
